...
var Weather = {
		...
		switch(num){
			...
			case 200:
				...
				for(var i = 1; i< 3; i++){
					...
					'<div class="w"><a href="http://tool.114la.com/tianqi/#{cityid}" target="_blank"><strong>#{week'+i+'}</strong><br />#{temp'+i+'}<br />#{weather'+i+'}</a></div></div>';
				}
	...
	getAreas: function(cid,callback){
		...
		Ylmf.ScriptLoader.Add({
            src: "http://weather.api.115.com/" + AreaId + "/" + AreaId + ".txt",
            charset: "utf-8"
        });
